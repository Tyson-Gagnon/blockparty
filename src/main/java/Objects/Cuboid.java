...
public class Cuboid implements Cloneable {
    ...
    protected final Vector3i minimumPoint, maximumPoint;
    ...
    public Cuboid(Location loc1, Location loc2) {
        if (loc1 != null && loc2 != null) {
            ...
            this.minimumPoint = new Vector3i(xPos1, yPos1, zPos1);
            this.maximumPoint = new Vector3i(xPos2, yPos2, zPos2);
        } else {
    ...
    public Cuboid(String worldName, double x1, double y1, double z1, double x2, double y2, double z2) {
        if (worldName == null || Sponge.getServer().getWorld(worldName) == null)
    ...
    public List<Location> getBlocks() {
        List<Location> blockList = new ArrayList<>();
            for (int x = this.minimumPoint.getX(); x <= this.maximumPoint.getX(); x++) {
                for (int y = this.minimumPoint.getY(); y <= this.maximumPoint.getY() && y <= 255; y++) {
                    for (int z = this.minimumPoint.getZ(); z <= this.maximumPoint.getZ(); z++) {
